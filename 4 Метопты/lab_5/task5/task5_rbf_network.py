...
def train(initial_params):
    ...
    # Убедимся, что sigma > 0
    result.x[2] = max(result.x[2], 0.01)
    result.x[5] = max(result.x[5], 0.01)
...
def main():
    ...
    plt.ylabel("Невязка z - ẑ")
    plt.title("Невязки по объектам для RBF-сети")
    plt.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig("task5_residuals.png", dpi=300)
    plt.close()
    ...
    print("   - task5_residuals.png (невязки)")
